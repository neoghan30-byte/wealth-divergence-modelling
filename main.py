...
if old_coeffsDict is None:
    old_coeffsDict, returnsDict, fullCorr, allTickersOrdered = m.getCoeffs(
    ...
    _, returnsDict, fullCorr, allTickersOrdered = m.getCoeffs(
        cfg["assets"], cfg["assetsCompleted"], cfg["assetsYahoo"], 
        old_assetWeights, cfg["households"], cfg["time"], 
        cfg["corrAbleClasses"], {}, old_inputParameters
    )
...
if baseline_bundle_path.exists() or baseline_bundle_path2.exists(): 
    ...
    try:
        assetResults = m.aggregate_to_asset_paths(
            TARGET_PATHS, 
            BASELINE_V_NUM, 
    ...
    # =====================================================================
    original_sorted_chunk_files = m._sorted_chunk_files
    ...
    m._sorted_chunk_files = patched_sorted_chunk_files
    ...
    metric_results_5k = m.get_metric_analysis(
...
new_baseline_comp = m.get_comparable_results(
    metric_results=metric_results_5k,
    name="baseline",
    inputParameters=old_inputParameters,
...
#         m.makeTablePretty(print_df, title.replace('_', ' '), graph_dir)
for title, tbl in tables.items():
    if not tbl.empty:
        print(f"=== {title.replace('_', ' ').upper()} ===")
