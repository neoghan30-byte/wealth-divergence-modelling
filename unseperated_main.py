...
def run_combined_analysis(
    ...
    # full mean path at each checkpoint => set TRACK_FULL_PATH = True below.
...
# def get_presentingAssetReturns(aggRes, households, assets):
...
#   meanCumAssetPaths = aggRes['portCumR']
#   for h in households:
#     presentingAssetReturns[h] = {assetCategory: 0.0 for assetCategory in assets} # Initialising with 0.0
...
#           if assetClass in meanCumAssetPaths[h] and tickerName in meanCumAssetPaths[h][assetClass]:
#             assetClassSeries = np.zeros(len(time))
#             if debugADC == True:
#               print(f"{assetClass}, {tickerName} is in meanCumAssetPaths")
#             presentingAssetReturns[h][broadCategory] = meanCumAssetPaths[h][assetClass][tickerName]
#             # if tickerName == "Land Other" and h == '80-100':
#             #  presentingAssetReturns[h][broadCategory] = assetClassSeries


#             # assetClassSeries += meanCumAssetPaths[h][assetClass][tickerName]
#             if debugADC == True:
#               print(f"WOAOIFASBFKBFKJ ================================ \n presentingAssetReturns[{h}][{broadCategory}] {presentingAssetReturns[h][broadCategory]}")
...
#           assetClass = source
#           if assetClass in meanCumAssetPaths[h]:
#             if debugADC == True:
#               print(f"{assetClass} is in meanCumAssetPaths")
#               print(f"{assetClass} in meanCumAssetPaths = {meanCumAssetPaths[h][assetClass].items()}")
#             assetClassSeries = np.zeros(len(time))
#             for tickerName in meanCumAssetPaths[h][assetClass]:
#               if debugADC == True:
#                 print(f"(meanCumAssetPaths[{h}][{assetClass}][{tickerName}])")
#                 print(f"{(meanCumAssetPaths[h][assetClass][tickerName])}")
#               assetClassSeries += (meanCumAssetPaths[h][assetClass][tickerName])
#             presentingAssetReturns[h][broadCategory] = assetClassSeries
...
def runGraphs(aggRes, assetResults, time, households, graph_dir, metric_results, tablesNeeded=True, plots_to_generate=None):
  ...
  if "validation" in metric_results_streamlined: # backtests
      ...
      if "band_results" in validation_results:
        ...
        sim_one_year = validation_results["convergence_results"].get("sim_one_year")
        hist_one_year = validation_results["convergence_results"].get("hist_one_year")
...
#   drive.mount('/content/drive')
  ...
  pathWoo = data_dir / "exampleFile.xlsx" #"/content/drive/MyDrive/Young_Economist/exampleFile.xlsx"
...
def aggregate_to_asset_paths(nTotalPaths, V_num):
  ...
  resultPath = data_dir / f"assetStateResults_{V_num}.pkl"# f"/content/drive/MyDrive/Young_Economist/assetStateResults_{V_num}.pkl"
...
housePricePath = data_dir / "HousingPricesYoungEcon.xlsx"
if os.path.exists(housePricePath):
  if debug6 == True:
   print("WOOO IT EXISTS. GOSH!!")
if not os.path.exists(housePricePath):
  raise FileNotFoundError("HousingPricesYoungEcon.xlsx not found")
